...
int main(int argc, char *argv[]) 
{
    ...
    while (nh.ok())
    {
        try
        {
            ...
            {
                int tuples = (tokens.size()/3);
		std::vector<visualeyez_tracker::TrackerPose> trackerPoses;
                for(int i=0;i<tuples;i++)
                {

                    trackerPose.header.stamp = ros::Time::now();
                    trackerPose.tracker_id   = tokens[3*i + 0];
                    trackerPose.pose.x       = atof(tokens[3*i + 1].c_str())/1000.0;
                    trackerPose.pose.y       = atof(tokens[3*i + 2].c_str())/1000.0;
                    trackerPose.pose.z       = atof(tokens[3*i + 3].c_str())/1000.0;
                    trackerPoses.push_back(trackerPose);
                    trackerPositionPublisher.publish(trackerPose);
                    ROS_INFO(" VisualEyez Sending Location: [%f] [%f] [%f]", trackerPose.pose.x ,trackerPose.pose.y ,trackerPose.pose.z);
                }
